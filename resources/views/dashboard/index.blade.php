...
<style>
    ...
    /* Pillar cards also stack on mobile */
    .pillar-grid {
        display: grid;
    ...
    /* Ensure cards never clip their content */
    .full-card {
        overflow: visible;
        display: flex;
    ...
    /* Scripture card min height so it never collapses */
    .scripture-card-wrap {
        display: flex;
        flex-direction: column;
    }

    /* Timer card — let it grow to fit all content */
    .timer-card-wrap {
        display: flex;
        flex-direction: column;
        height: 100%;
...
<div x-data="dashboardApp()">
    ...
    <div style="background:linear-gradient(135deg,#1a2c5b 0%,#2a3f7a 60%,#0f3460 100%);border-radius:24px;padding:32px 36px;margin-bottom:28px;position:relative;overflow:hidden;" class="fade-in-up">
        ...
        <div style="position:relative;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:20px;">
            <div>
                <p style="font-family:'Cinzel',serif;font-size:0.72rem;letter-spacing:3px;text-transform:uppercase;color:rgba(212,160,23,0.7);margin-bottom:6px;">YOUR CENTURION JOURNEY</p>
                <h2 style="font-family:'Cinzel Decorative',serif;font-size:1.8rem;color:#f5d060;line-height:1.2;margin-bottom:8px;">{{ auth()->user()->full_name }}</h2>
                <p style="color:rgba(255,255,255,0.5);font-size:0.88rem;">{{ auth()->user()->church }} · {{ auth()->user()->group }}</p>

                <div style="display:flex;gap:10px;margin-top:16px;flex-wrap:wrap;">
                    @if(auth()->user()->overall_progress >= 100)
                    <span style="background:linear-gradient(135deg,#d4a017,#f5d060);color:#1a2c5b;padding:6px 16px;border-radius:50px;font-family:'Cinzel',serif;font-size:0.72rem;font-weight:700;">👑 FULL CENTURION</span>
                    @elseif(auth()->user()->overall_progress >= 50)
                    <span style="background:rgba(212,160,23,0.15);color:#f5d060;border:1px solid rgba(212,160,23,0.3);padding:6px 16px;border-radius:50px;font-family:'Cinzel',serif;font-size:0.72rem;">⚡ ADVANCING</span>
                    @else
                    <span style="background:rgba(255,255,255,0.08);color:rgba(255,255,255,0.5);border:1px solid rgba(255,255,255,0.1);padding:6px 16px;border-radius:50px;font-family:'Cinzel',serif;font-size:0.72rem;">🌱 STARTED</span>
                    @endif
                </div>
            </div>
    ...
    <div class="pillar-grid">
        ...
        <!-- Prayer Pillar -->
        <div class="centurion-card full-card fade-in-up">
    ...
    <!-- Scripture + Prayer Timer -->
    <div class="dashboard-bottom-row">

        <!-- Daily Scripture -->
        <div class="scripture-card scripture-card-wrap fade-in-up">
            ...
            <div style="margin-top:16px;padding-top:16px;border-top:1px solid rgba(212,160,23,0.1);">
                <p style="font-size:0.75rem;color:rgba(255,255,255,0.35);font-family:'Cinzel',serif;">{{ now()->format('F j, Y') }}</p>
            </div>
        </div>

        <!-- Quick Prayer Timer -->
        <div class="centurion-card timer-card-wrap fade-in-up" style="padding:24px;" x-data="prayerTimer()">
            ...
            <div style="text-align:center;padding:20px 0;">
                ...
                <p style="font-family:'Cinzel',serif;font-size:0.72rem;color:#9ca3af;margin-top:8px;letter-spacing:1px;">HH : MM : SS</p>
            </div>
            ...
            <div x-show="running || paused" x-transition
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
                    ...
                    <div style="display:flex;gap:6px;flex-shrink:0;">
                        <button @click="prevSong()" style="width:28px;height:28px;border-radius:50%;background:rgba(255,255,255,0.1);border:none;color:white;cursor:pointer;font-size:0.7rem;display:flex;align-items:center;justify-content:center;">⏮</button>
                        <button @click="nextSong()" style="width:28px;height:28px;border-radius:50%;background:rgba(255,255,255,0.1);border:none;color:white;cursor:pointer;font-size:0.7rem;display:flex;align-items:center;justify-content:center;">⏭</button>
                        <button @click="toggleMute()" style="width:28px;height:28px;border-radius:50%;background:rgba(255,255,255,0.1);border:none;color:white;cursor:pointer;font-size:0.7rem;display:flex;align-items:center;justify-content:center;"><span x-text="muted ? '🔇' : '🔊'"></span></button>
                    </div>
                </div>

                <div style="margin-top:10px;">
                    <div style="background:rgba(255,255,255,0.1);height:3px;border-radius:2px;overflow:hidden;">
                        <div :style="`width:${songProgress}%;`" style="height:100%;background:linear-gradient(90deg,#f5d060,#fb923c);border-radius:2px;transition:width 1s linear;"></div>
                    </div>
                ...
                <div style="margin-top:10px;display:flex;flex-direction:column;gap:4px;">
                    <template x-for="(song, index) in songs" :key="index">
                        <div @click="playSong(index)"
                            ...
                            <span :style="currentSong === index && running ? 'color:#f5d060;' : 'color:rgba(255,255,255,0.3);'" style="font-size:0.65rem;">▶</span>
                            <span x-text="song.title" :style="currentSong === index ? 'color:#f5d060;' : 'color:rgba(255,255,255,0.5);'" style="font-family:'Cinzel',serif;font-size:0.68rem;"></span>
                        </div>
                    </template>
                </div>
            </div>
            ...
            <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap;">
                <button @click="start()" x-show="!running" class="cd-btn cd-btn-gold" style="font-size:0.8rem;"><i class="fas fa-play"></i> Start Prayer</button>
                <button @click="pause()" x-show="running && !paused" class="cd-btn" style="background:#fef3c7;color:#92400e;font-size:0.8rem;"><i class="fas fa-pause"></i> Pause</button>
                <button @click="resume()" x-show="paused" class="cd-btn cd-btn-gold" style="font-size:0.8rem;"><i class="fas fa-play"></i> Resume</button>
                <button @click="saveSession()" x-show="elapsed > 0" class="cd-btn cd-btn-primary" style="font-size:0.8rem;"><i class="fas fa-save"></i> Save</button>
                <button @click="reset()" x-show="elapsed > 0" class="cd-btn" style="background:#f3f4f6;color:#6b7280;font-size:0.8rem;"><i class="fas fa-redo"></i></button>
            </div>

            <div x-show="saved" x-transition style="margin-top:16px;background:#f0fdf4;border:1px solid #86efac;border-radius:10px;padding:10px 14px;text-align:center;">
    ...
    <div class="dashboard-activity-row">
        ...
        <!-- Recent Activity -->
        <div class="centurion-card full-card fade-in-up" style="padding:24px;">
            ...
            <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid rgba(212,160,23,0.06);">
                <div style="width:36px;height:36px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:1rem;flex-shrink:0;
                ...
                <div style="flex:1;min-width:0;">
                    <p style="font-family:'Cinzel',serif;font-size:0.8rem;color:#374151;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $activity->title }}</p>
                    <p style="font-size:0.72rem;color:#9ca3af;margin-top:2px;">{{ $activity->created_at->diffForHumans() }}</p>
                </div>
                <span style="font-family:'Cinzel',serif;font-size:0.78rem;color:#1a2c5b;font-weight:700;flex-shrink:0;">{{ $activity->value }}</span>
            </div>
            @empty
        ...
        <div class="centurion-card full-card fade-in-up" style="padding:24px;">
            ...
            <div class="lb-row">
                ...
                <div style="flex:1;min-width:0;">
                    <p style="font-family:'Cinzel',serif;font-size:0.8rem;color:#374151;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ Str::limit($user->full_name, 16) }}</p>
                    <p style="font-size:0.68rem;color:#9ca3af;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $user->church }}</p>
                </div>
...
function prayerTimer() {
    return {
        ...
        init() {
            this.$nextTick(() => {
                ...
                this.audio.addEventListener('timeupdate', () => {
                    ...
                    this.songProgress = this.songDuration ? (this.songElapsed / this.songDuration) * 100 : 0;
                });
            });
        },
        ...
        reset() {
            ...
            this.elapsed = 0;    this.saved  = false;
            this.audio.pause();
            this.audio.currentTime = 0;
            this.songElapsed = 0; this.songDuration = 0; this.songProgress = 0;
        },
        saveSession() {
            clearInterval(this.interval);
            this.running = false; this.paused = false;
        ...
        playSong(index) { this.currentSong = index; this.playCurrentSong(); },
        nextSong() { this.currentSong = (this.currentSong + 1) % this.songs.length; this.playCurrentSong(); },
        prevSong() { this.currentSong = (this.currentSong - 1 + this.songs.length) % this.songs.length; this.playCurrentSong(); },
        toggleMute() { this.muted = !this.muted; this.audio.muted = this.muted; },
        formatTime(s) {
